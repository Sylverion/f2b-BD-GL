...
void Render::checkAndSaveTexture(int16_t texKey, const uint8_t* texData, int texW, int texH) {
	if (savedTextures.count(texKey) == 0) {
		char filename[128];
		snprintf(filename, sizeof(filename), "texture_%d.png", texKey);

		// Get the cached texture
		Texture* t = _textureCache.getCachedTexture(texKey, texData, texW, texH, true);
		...
		auto saveTextureAsPng = [&](const char* filename, const uint8_t* data, int width, int height) {
			if (!stbi_write_png(filename, width, height, 4, data, width * 4)) {
				std::cerr << "Failed to save texture to PNG" << std::endl;
			}
		};

		saveTextureAsPng(filename, pixels.data(), width, height);

		// Mark the texture as saved
